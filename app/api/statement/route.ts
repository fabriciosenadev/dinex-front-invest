...
export async function GET(request: Request) {
  try {
    ...
    const target = query ? `${apiBaseUrl}/api/statement?${query}` : `${apiBaseUrl}/api/statement`;
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${apiBaseUrl}/api/statement`, {
      method: "POST",
      headers: {
...
export async function DELETE(request: Request) {
  try {
    ...
    const response = await fetch(`${apiBaseUrl}/api/statement/all`, {
      method: "DELETE",
      headers: {
        Authorization: authHeader
      },
